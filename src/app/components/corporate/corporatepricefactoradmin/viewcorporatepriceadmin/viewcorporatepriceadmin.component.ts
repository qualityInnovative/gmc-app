...
@Component({
  selector: 'ratelist-viewcorporatepriceadmin',
  templateUrl: './viewcorporatepriceadmin.component.html',
  styleUrls: ['./viewcorporatepriceadmin.component.scss']
})
export class ViewcorporatepriceadminComponent implements OnInit {
  edit: boolean = false;
  faEye = faEye;
  state: State[] = [];
  districts: District[] = [];
  ...
  addorupdatecorporatepricefactor() {
    console.log(this.corporatepricefactor);
    if (this.edit) {
      this.corporationdiscountService.updatecorporatepricefactoradmin(this.corporatepricefactor).subscribe((res: any) => {
        console.log(res);
        this.location.back();
      }, (error: any) => {
        console.log(error);
  ...
  isApprovedSelected(Event: any) {
    this.corporatepricefactor.isApproved = Event
    console.log(this.corporatepricefactor.isApproved);
  }
